...
$(function () {
    if (window.innerWidth <= 900) {
        document.querySelector('.smallerScreensModal').style.display = 'flex';
    } else {
        ...
        $("#proceedGameBtn").on('click', function() {
        ...
        function finalStatistics() {
            var letterSForKiosks = '';
            var letterSForShops = '';
            var letterSForBanks = '';
            gameData.kiosks === 1?'':letterSForKiosks='s';
            gameData.shops === 1?'':letterSForShops='s';
            gameData.banks === 1?'':letterSForBanks='s';
            $('.kiosksOwnedFinal').text(gameData.kiosks + ' kiosk' + letterSForKiosks);
            $('.shopsOwnedFinal').text(gameData.shops + ' shop' + letterSForShops);
            $('.banksOwnedFinal').text(gameData.banks + ' bank' + letterSForBanks);
        }
